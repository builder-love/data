-- macros/generate_normalized_contributor_dominant_language.sql

{% macro generate_normalized_contributor_dominant_language(
    contributor_limit
) %}

{% set initial_load_timestamp = '2025-06-26T08:59:00.030134Z' %}
...
{% if is_incremental() %}
    ...
    language_counts as (
        ...
        WHERE po.contributor_rank <= {{ contributor_limit }}
        and po.data_timestamp in (select load_timestamps from load_timestamps)
...
{% else %}
    ...
    FROM {{ ref('latest_top_contributors') }} po 
    WHERE po.contributor_rank <= {{ contributor_limit }}
    and po.data_timestamp = '{{ initial_load_timestamp }}'::timestamp
    and dominant_language is not NULL
    group by 1
    order by 2 desc
