...
{% set initial_load_timestamp = '2025-05-21T19:58:18.497642Z' %}
...
{% if is_incremental() %}
    ...
    raw_data_timestamps as (
        ...
        from {{ source('raw', 'project_repos_is_fork') }}
        group by 1
    ),
    ...
    SELECT
        po.*
    FROM
        {{ source('raw', 'project_repos_is_fork') }} po
