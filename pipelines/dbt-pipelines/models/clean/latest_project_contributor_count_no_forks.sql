-- models/clean/latest_project_contributor_count_no_forks.sql
-- if clean.latest_project_contributor_count_no_forks table exists, replace it with the latest data from clean.latest_contributors and clean.latest_project_repos_contributors tables
...
with latest_project_repos_contributors as (
  ...
  left join {{ ref('latest_project_repos_is_fork') }} f on lprc.repo = f.repo
...
project_contributor_count as (
  ...
  from latest_project_repos_contributors contributors left join {{ ref('latest_project_repos') }} lpr
