-- normalized_project_repos_watcher_count.sql
-- create an incremental table in clean with data_timestamp intervals >6 days apart
-- project_repos_watcher_count raw table
-- 
{{ config(
...
{% set initial_load_timestamp = '2025-04-07T17:16:49.031257Z' %}
...
{% if is_incremental() %}
    ...
    raw_data_timestamps as (
        ...
        from {{ source('raw', 'project_repos_watcher_count') }}
        group by 1
    ),
    ...
    SELECT
        po.*
    FROM
        {{ source('raw', 'project_repos_watcher_count') }} po
