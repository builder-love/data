...
{% set initial_load_timestamp = '2025-03-06T00:20:24.040912Z' %}
...
{% if is_incremental() %}
    ...
    raw_data_timestamps as (
        ...
        from {{ source('raw', 'project_repos_fork_count') }}
        group by 1
    ),
    ...
    SELECT
        po.*
    FROM
        {{ source('raw', 'project_repos_fork_count') }} po
