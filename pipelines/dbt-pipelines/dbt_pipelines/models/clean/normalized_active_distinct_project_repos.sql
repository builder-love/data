-- normalized_active_distinct_project_repos.sql
-- create an incremental table in clean with data_timestamp intervals >25 days apart
-- project_repos_active_distinct_count raw table
-- 
{{ config(
...
{% set initial_load_timestamp = '2025-03-21T11:46:06.682445Z' %}
...
{% if is_incremental() %}
    ...
    raw_data_timestamps as (
        ...
        from {{ source('clean', 'latest_active_distinct_project_repos') }}
        group by 1
    ),
    ...
    SELECT
        po.*
    FROM
        {{ source('clean', 'latest_active_distinct_project_repos') }} po
