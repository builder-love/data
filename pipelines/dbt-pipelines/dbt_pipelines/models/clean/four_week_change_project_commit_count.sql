...
LatestWeeklyProjectCommitCount AS (
    -- Step 2: Get the commit count associated with that latest timestamp per week
    SELECT
...
LaggedWeeklyCounts AS (
    ...
    SELECT
        ...
        project_title,
        commit_count,
        max_timestamp_in_week, -- The timestamp for the current 'commit_count'
        LAG(commit_count, 4) OVER (
    ...
    FROM 
        LatestWeeklyProjectCommitCount
),
