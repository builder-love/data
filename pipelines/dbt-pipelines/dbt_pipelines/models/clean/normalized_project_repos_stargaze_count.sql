-- normalized_project_repos_stargaze_count.sql
...
{% set initial_load_timestamp = '2025-03-05T17:29:29.552079Z' %}
...
{% if is_incremental() %}
    ...
    raw_data_timestamps as (
        ...
        from {{ source('raw', 'project_repos_stargaze_count') }}
        group by 1
    ),
    ...
    SELECT
        po.*
    FROM
        {{ source('raw', 'project_repos_stargaze_count') }} po
