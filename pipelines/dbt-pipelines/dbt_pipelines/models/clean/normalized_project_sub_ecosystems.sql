...
{% set initial_load_timestamp = '2025-02-12T07:27:25.262443Z' %}
...
{% if is_incremental() %}
    ...
    raw_data_timestamps as (
        ...
        from {{ source('raw', 'project_sub_ecosystems') }}
        group by 1
    ),
    ...
    SELECT
        po.*
    FROM
        {{ source('raw', 'project_sub_ecosystems') }} po
