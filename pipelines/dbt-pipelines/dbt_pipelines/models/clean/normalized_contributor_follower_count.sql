...
{% set initial_load_timestamp = '2025-05-22T13:20:15.179641Z' %}
...
{% if is_incremental() %}
    ...
    raw_data_timestamps as (
        ...
        from {{ source('raw', 'contributor_follower_count') }}
        group by 1
    ),
    ...
    SELECT
        po.*
    FROM
        {{ source('raw', 'contributor_follower_count') }} po
